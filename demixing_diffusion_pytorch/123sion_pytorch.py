...
class Dataset_Aug1(data.Dataset):
    def __init__(self, folder1, folder2, image_size, exts = ['jpg', 'jpeg', 'png']):
        ...
        self.annot_path = os.path.join('/seu_share/home/luxiaobo/228465/demixing-diffusion-pytorch/', 'kitti_3dop_train.json')
    ...
    def __getitem__(self, index):
        ...
        path1 = os.path.join(self.folder1, img_info['file_name'])
        path2 = os.path.join(self.folder2, img_info['file_name'])
        #print(path1)
        #print(path2)
        ...
        if f[0]<=50:
            ...
            img1 = img1.convert('RGB')
            img1 = self.transformf(img1)
            img2 = img2.convert('RGB')
            img2 = self.transformf(img2)
...
class Dataset(data.Dataset):
    def __init__(self, folder1,folder2, image_size, exts=['jpg', 'jpeg', 'png']):
        ...
        self.annot_path = os.path.join('/seu_share/home/luxiaobo/228465/demixing-diffusion-pytorch/', 'kitti_3dop_val.json')
        self.coco = coco.COCO(self.annot_path)
        self.images = self.coco.getImgIds()
        self.num_samples = len(self.images)
        #imdb_file_path = os.path.join('/home/member/syg/visualDet3D/workdirs/PStereo3D/output/', 'training', 'imdb.pkl')
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        if dataset == 'train':
            ...
            self.dl1= cycleval(data.DataLoader(self.ds, batch_size = train_batch_size, shuffle=shuffle, pin_memory=True, num_workers=16, drop_last=True))
        ...
        #self.opt = AdamW(diffusion_model.parameters())
        self.step = 0
    ...
    def train(self):
        #experiment = Experiment(api_key="57ArytWuo2X4cdDmgU1jxin77", project_name="Cold_Diffusion_Cycle")
        ...
        while self.step < self.train_num_steps:
            ...
            for i in range(self.gradient_accumulate_every):
                ...
                #utils.save_image(data_1, str(self.results_folder / f'data_1-og-{i}.png'), nrow=6)
                #utils.save_image(data_2, str(self.results_folder / f'data_2-og-{i}.png'), nrow=6) 
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
               # #######exasdperiment.log_dfurrent_epoch(self.step)
                milestone = self.step // self.save_and_sample_every
                batches = self.batch_size
                #og_img = next(self.dl2).cuda()
                #og_img1 = next(self.dl1).cuda()
                og_img1,og_img2, name = next(self.dl1)
                ...
                utils.save_image(og_img1, str(self.results_folder / f'sample-R-{milestone}.png'), nrow=6)

                all_images = (all_images + 1) * 0.5
                utils.save_image(all_images, str(self.results_folder / f'sample-PR-{milestone}.png'), nrow = 6)
    ...
    def sample_and_save_for_fid(self, noise=0):
        ...
        for j in range(int(3796/self.batch_size)):
            #og_img = next(self.dl2).cuda()
            #print(og_img.shape)
            og_img1,og_img2,name = next(self.dl1)
            #og_img1 = og_img1.cuda() #R
            ...
            for i in range(all_images.shape[0]):
                #all_images[i]
                utils.save_image((resize(all_images[i]) ) , str(f'{out_folder}/' + f'{"".join(name)[:-4] }.png'), range=None)
