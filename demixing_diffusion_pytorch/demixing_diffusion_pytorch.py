...
class ConvNextBlock(nn.Module):
    ...
    def __init__(self, dim, dim_out, *, time_emb_dim = None, mult = 2, norm = True):
        ...
        if dim == 3:
          ...
          self.ds_conv = InceptionDWConv2d(dim)
        ''' 
        self.ds_conv = nn.Conv2d(dim, dim, 7, padding = 3, groups = dim)
...
class DeformConv(nn.Module):
    def __init__(self, cho):
        ...
        self.conv = DCN(cho, cho, kernel_size=(3, 3), stride=1, padding=1, dilation=1, deformable_groups=1)
...
class Unet(nn.Module):
    ...
    def forward(self, x, time):
        ...
        for convnext, convnext2, attn, upsample in self.ups: #, dcn
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,   
        ...
        self.sample_step = self.num_timesteps
        ...
        midsteps = torch.arange(self.num_timesteps - 1, 1,step=-((self.num_timesteps - 1) // (self.sample_step - 2)), dtype=torch.long)
        ...
        m_t = np.linspace(m_min, m_max, self.num_timesteps)
    ...
    @torch.no_grad()
    def sample(self, batch_size = 16, img=None,img64=None, t=None):
        ...
        while (t):
            ...
            #img1 = self.vqgan.encode(img)
            x1_bar = self.denoise_fn(img, step)
            #x1_bar = self.vqgan.decode(x1_bar)
    ...
    def p_sample(self, x_t, y, context, i, clip_denoised=False):
        ...
        if self.steps[i] == 0:
            ...
            sigma_t = torch.sqrt(sigma2_t) *0.9# 1.0
    ...
    @torch.no_grad()
    def p_sample_loop(self, y, context=None, clip_denoised=False, sample_mid_step=False):
        '''
        ...
        if sample_mid_step:
            ...
            for i in tqdm(range(self.num_timesteps), desc=f'sampling loop time step', total=self.num_timesteps):
    ...
    def bbdm_sample(self, x0, y, t, noise=None):
    ...
    def flow_sample(self, x0, device='cuda'):
        self.denoise_fn.eval()
        with torch.no_grad():
            rtol = atol = 1e-5
            method = 'RK45'
            ...
            def ode_func(t, x):
                ...
                drift = self.denoise_fn(x, vec_t * 999)

                return drift.detach().cpu().numpy().reshape((-1,))

            # Black-box ODE solver for the probability flow ODE
            solution = integrate.solve_ivp(ode_func, (eps, T), x,
    ...
    def p_losses(self, x_start, x_end, t, noise=None):
        ...
        if self.train_routine == 'Final':
            ...
            lossssim = 1- ssim(x_start, x_recon, data_range=1, size_average=True)
            loss =3 *loss1 + 2 *lossssim #+ lpips_losses

            ''' 
            # bbdm
            noise = default(noise, lambda: torch.randn_like(x_start))
            x_t, objective = self.bbdm_sample(x_start, x_end, t, noise)
            objective_recon = self.denoise_fn(x_t, t)
            loss1 = (objective - objective_recon).abs().mean()
            x0_recon = x_t - objective_recon
            lossssim = 1- ssim(x_start, x0_recon, data_range=1, size_average=True)
            loss =3 *loss1 + 3 *lossssim 
            ...
            T = 1.
            eps=1e-3
            t_flow = torch.rand(x_start.shape[0], device=x_start.device) * (T - eps) + eps
            t_expand = t_flow.view(-1, 1, 1, 1).repeat(1, x_start.shape[1], x_start.shape[2], x_start.shape[3])
            perturbed_data = t_expand * x_start + (1.-t_expand) * x_end #z-noize batch-dedao  z-L batch-R
            target = x_start - x_end
            
            score = self.denoise_fn(perturbed_data, t*999) ### Copy from models/utils.py
            p_r = x_end + score
            loss2 = torch.square(score - target).mean()
            lossssim = 1- ssim(x_start, p_r, data_range=1, size_average=True)
            loss = loss2 + lossssim 
            #loss = reduce_op(losses.reshape(losses.shape[0], -1), dim=-1)
            '''
            ...
            if self.loss_type == 'l1':
                ...
                # #lossssim = 1- ssim(x_start, x_recon, data_range=1, size_average=True)
                # loss1 = (objective - objective_recon).abs().mean()
                # #x0_recon = self.predict_x0_from_objective(x_t, x_end, t, objective_recon)
                # x0_recon = x_t - objective_recon
                # lossssim = 1- ssim(x_start, x0_recon, data_range=1, size_average=True)
                # #lpips_losses = self.lpips_model(x_recon, x_start).view(x_start.shape[0], 1)
    ...
    def autop_losses(self, x_start, x_end):
        b, c, h, w = x_start.shape
        if self.train_routine == 'Final':
            quant, diff, _  = self.vqgan.encode(x_start)
            
            dec = self.vqgan.decode(quant)
            
            piploss, _ = self.piploss(out, x_start, x_latent, 0, self.global_step,
                                            last_layer=self.vqgan.decoder.conv_out.weight, split="train")
            
            lpips_losses = self.lpips_model(dec, x_start).view(x_start.shape[0], 1)
    ...
    def forward(self, x1, x2, *args, **kwargs):
        ...
        if task:
          ...
          return self.autop_losses(x1, x2, *args, **kwargs)
        
import pickle
import pycocotools.coco as coco
class Dataset_Aug1(data.Dataset):
    def __init__(self, folder1, folder2, image_size, exts = ['jpg', 'jpeg', 'png']):
        super().__init__()
        self.folder1 = folder1
        self.folder2 = folder2
        self.image_size = image_size
        self.paths1 = [p for ext in exts for p in Path(f'{folder1}').glob(f'**/*.{ext}')]
        self.paths2 = [p for ext in exts for p in Path(f'{folder2}').glob(f'**/*.{ext}')]

        self.transformr = transforms.Compose([
        ...
        self.transformrf = transforms.Compose([
        ...
        self.transformlf = transforms.Compose([
            transforms.RandomHorizontalFlip(p=1),
            transforms.Resize((int(image_size[0]), int(image_size[1]))),
            transforms.ToTensor(),
            #transforms.Lambda(lambda t: (t * 2) - 1)
        ...
        '''
        #self.annot_path = os.path.join('/home/member/syg/cold-Diffusion/demixing-diffusion-pytorch/', 'kitti_3dop_train.json')
        self.annot_path = os.path.join('/seu_share/home/luxiaobo/228465/demixing-diffusion-pytorch/', 'kitti_kitti_train.json')
        self.coco = coco.COCO(self.annot_path)
        self.images = self.coco.getImgIds()
        self.num_samples = len(self.images)
        #imdb_file_path = os.path.join('/home/member/syg/visualDet3D/workdirs/PStereo3D/output/', 'training', 'imdb.pkl')
    ...
    def __getitem__(self, index):
        ...
        #path2 = os.path.join(self.folder1, img_info['file_name'])
        #path1 = path2.replace('image_02', 'image_03')
        path1 = os.path.join(self.folder1, img_info['file_name_r'])
        path2 = os.path.join(self.folder2, img_info['file_name_l'])
        fid = img_info['fid']
        #print(path1)
        #print(path2)
        #f = np.random.randint(1,100,1)
        ...
        if fid==0:
            img1 = self.transformr(img1)
            img2 = self.transformr(img2)
        else:
            img1 = self.transformrf(img1)
            img2 = self.transformrf(img2)
...
class Dataset(data.Dataset):
    def __init__(self, folder1,folder2, image_size, exts=['jpg', 'jpeg', 'png']):
        ...
        '''
        #self.annot_path = os.path.join('/seu_share/home/luxiaobo/228465/demixing-diffusion-pytorch/', 'kitti_kitti_trainval.json')
        self.annot_path = os.path.join('./', 'kitti_obj_test.json') #kitti_obj_val
        self.coco = coco.COCO(self.annot_path)
        self.images = self.coco.getImgIds()
        self.num_samples = len(self.images)
        #imdb_file_path = os.path.join('/home/member/syg/visualDet3D/workdirs/PStereo3D/output/', 'training', 'imdb.pkl')
    ...
    def __getitem__(self, index):
        index = 7517-index#+1792
        img_id = self.images[index]
        img_info = self.coco.loadImgs(ids=[img_id])[0]

        #path1 = os.path.join('/seu_share2/datasets/kitti/training/image_3/', img_info['file_name'])
        path2 = os.path.join('/seu_share2/datasets/kitti/testing/image_2/', img_info['file_name'])
        #fid = img_info['fid']
        #print(path1)
        #print(path2)
        #img1 = Image.open(path1)
        ...
        #return (img1, img2, img_info['file_name'])
        return (img2, img_info['file_name'],w,h)
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        if dataset == 'train':
            ...
            self.ds= Dataset_Aug1(folder1, folder2, image_size)
            self.dl1= cycle(data.DataLoader(self.ds, batch_size = train_batch_size, shuffle=shuffle, pin_memory=True, drop_last=True,num_workers=4 ))
            ...
            self.dl1= cycleval(data.DataLoader(self.ds, batch_size = train_batch_size, shuffle=False, pin_memory=True, drop_last=False,num_workers=4))
            #self.num_samples = Dataset.__len__(self)
            #self.ds2 = Dataset(folder2, image_size)cycleval

        #self.dl1= cycle(data.DataLoader(self.ds, batch_size = train_batch_size, shuffle=shuffle, pin_memory=True, num_workers=16, drop_last=True))
        
        #self.dl2 = cycle(data.DataLoader(self.ds[1], batch_size=train_batch_size, shuffle=shuffle, pin_memory=True, num_workers=16, drop_last=True))

        #self.opt = Adam(diffusion_model.parameters(), lr=train_lr)
        self.opt = AdamW(diffusion_model.parameters(), lr=train_lr, weight_decay=0)
        self.step = 0
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            for i in range(self.gradient_accumulate_every):
                #print(self.dl1)
                #data_1 = np.array([self.dl1["image"][0]]).cuda()
                data_1,data_2 = next(self.dl1)
                data_1 = data_1.cuda() #R
                data_2 = data_2.cuda() #L
                #data_2 = next(self.dl2).cuda()

                #mean = torch.as_tensor([0.485, 0.456, 0.406]).view(-1, 1, 1).cuda()
                #std = torch.as_tensor([0.229, 0.224, 0.225]).view(-1, 1, 1).cuda()
                #utils.save_image(data_1*std+mean, str(self.results_folder / f'data_1-og-{i}.png'), nrow=6)
                #utils.save_image(data_2*std+mean, str(self.results_folder / f'data_2-og-{i}.png'), nrow=6) 
                #assert False 
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0: #     :#
                ...
                #og_img1 = next(self.dl1).cuda()
                og_img1,og_img2 = next(self.dl1)
                og_img1 = og_img1.cuda() #R
                og_img2 = og_img2.cuda() #L

                #all_images = self.ema_model.module.p_sample_loop(og_img2) 
                xt, direct_recons, all_images = self.ema_model.module.sample(batch_size=batches, img=og_img2) 
                #all_images = self.ema_model.module.flow_sample(og_img2) 
                
                
                
                mean = torch.as_tensor([0.485, 0.456, 0.406]).view(-1, 1, 1).cuda()
                std = torch.as_tensor([0.229, 0.224, 0.225]).view(-1, 1, 1).cuda()
                og_img1 =((og_img1+ 1) * 0.5) *std+mean
                utils.save_image(og_img1, str(self.results_folder / f'sample-R-{milestone}.png'), nrow=4)
 
                all_images = ((all_images+ 1) * 0.5) *std+mean
                utils.save_image(all_images, str(self.results_folder / f'sample-PR-{milestone}.png'), nrow = 4)

                #direct_recons = (direct_recons *std+mean+ 1) * 0.5
                #utils.save_image(direct_recons, str(self.results_folder / f'sample-direct_R-{milestone}.png'), nrow=6)

                #xt = (xt *std+mean+ 1) * 0.5
                xt = ((og_img2+ 1) * 0.5) *std+mean
                utils.save_image(xt, str(self.results_folder / f'sample-L-{milestone}.png'),
                                 nrow=4)
    ...
    def sample_and_save_for_fid(self, noise=0):
        ...
        cnt = 0
        bs = self.batch_size
        #resize=transforms.Compose([transforms.Resize((375,1242))])
        imgnum = 7482
        tot =imgnum +self.batch_size-1
        for j in range(tot//self.batch_size):
            #og_img = next(self.dl2).cuda()
            #print(og_img.shape)
            #og_img1,og_img2,name,size = next(self.dl1)
            og_img2,name,W,H  = next(self.dl1)
            #og_img1 = og_img1.cuda() #R
            og_img = og_img2.cuda() #L

            xt, direct_recons, all_images = self.ema_model.module.gen_sample(batch_size=bs, img=og_img, noise_level=noise)
            #all_images =self.ema_model.module.p_sample_loop(og_img)
            #all_images = self.ema_model.module.flow_sample(og_img2) 
            ...
            std = torch.as_tensor([0.229, 0.224, 0.225]).view(-1, 1, 1).cuda()
                #utils.save_image(data_1*std+mean, str(self.results_folder / f'data_1-og-{i}.png'), nrow=6)
                #utils.save_image(data_2*std+mean, str(self.results_folder / f'data_2-og-{i}.png'), nrow=6) 
            #all_images = F.interpolate(all_images, size=[size[1], size[0]], mode="bicubic")
            for i in range(all_images.shape[0]):
                #all_images[i] 
                #utils.save_image((resize(all_images[i]*std+mean)) , str(f'{out_folder}/' + f'{"".join(name[i])[:-4] }.png'), range=None)
                p_right = ((all_images[i]+ 1) * 0.5) *std+mean
                Resize = transforms.Resize([H[i], W[i]], interpolation=3)
                p_right = Resize(p_right)
                utils.save_image(p_right, str(f'{out_folder}/' + f'{"".join(name[i])[:-4] }.png'))
...
import math
import os
# from src.lib.models.registry import Registry
...
def get_model_url(data='imagenet', name='dla34', hash='ba72cf86'):
    # return join('http://dl.yf.io/dla/models/imagenet/dla60x-d15cacda.pth')
...
class DLASeg(nn.Module):
    def __init__(self, base_name,
        ...
        #self.landr_Conv0 = nn.Conv2d(32, 3, 1)
        #nn.Conv2d(dim, out_dim, 1)
        #self.landr_Conv0 = nn.Conv2d(32, 3, kernel_size=1, stride=2, padding=30, bias=True)
        # self.fc = nn.Sequential(
        #     nn.Conv2d(channels[self.first_level], 3, kernel_size=1,
        #               stride=1, padding=1, bias=True)
        # )
        self.fc = nn.ConvTranspose2d(channels[self.first_level], 3, kernel_size=(2, 2), stride=(2, 2))

    def forward(self, x1):
        x1 = self.base(x1)
        x1 = self.dla_up(x1[self.first_level:])
        
        return self.fc(x1)

def main():
    x = torch.randn(1, 3, 192, 640)
    #y = torch.randn(1, 3, 384, 1280)
    model = DLASeg('dla{}'.format(34),
                   pretrained=False,
                   down_ratio=2
                   )
    out = model(x)
